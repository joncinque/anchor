            ...
            init: init.as_ref().map(|i| Ok(ConstraintInitGroup {
                ...
                kind: if let Some(tm) = &token_mint {
                    InitKind::Token {
                        ...
                        token_program: token_token_program.map(|tp| tp.clone().into_inner().token_program),
                    }
                } else if let Some(at) = &associated_token {
                    InitKind::AssociatedToken {
                        mint: at.mint.clone(),
                        owner: at.wallet.clone(),
                        token_program: associated_token_program.map(|tp| tp.clone().into_inner().token_program),
                    }
                    ...
                    InitKind::Mint {
                        ...
                        token_program: mint_token_program.map(|tp| tp.clone().into_inner().token_program),
                    }
                } else {
                    InitKind::Program {
